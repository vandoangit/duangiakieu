        ...
        validateCall:function(caller,rules){	// EXECUTE VALIDATION REQUIRED BY THE USER FOR THIS FIELD
            ...
            for(i=0;i < rules.length;i++){
                switch(rules[i]){
                    ...
                    case "confirm":
                        if((!$.validationEngine.isError) && ($("#" + rules[i + 1]).attr('value') != '')){
            ...
            function content(caller,rules,position){    	  // VALIDATE LENGTH
                ...
                if(callerType == "text" || callerType == "password" || callerType == "textarea"){
                    ...
                    if(!$(caller).val()){
                        ...
                        promptText+=rules[position + 1] + $.validationEngine.settings.allrules[rules[i]].alertText + "<br />";
                    }
                }
            }
            ...
            function _required(caller,rules){   // VALIDATE BLANK FIELD
                ...
                if(callerType == "text" || callerType == "password" || callerType == "textarea" || callerType == undefined){
            ...
            function _customRegex_vi(caller,rules,position){		 // VALIDATE REGEX RULES
                customRule=rules[position + 1];
                pattern=eval($.validationEngine.settings.allrules[customRule].regex);

                if(!pattern.test(convert_vi_en($(caller).attr('value')))){
                    $.validationEngine.isError=true;
                    promptText+=$.validationEngine.settings.allrules[customRule].alertText + "<br />";
                }
            }
            ...
            function _customRegex_select(caller,rules,position){		 // VALIDATE REGEX RULES
                customString=rules[position + 1];
                if(customString == $(caller).attr('value')){
                    $.validationEngine.isError=true;
                    promptText+=$.validationEngine.settings.allrules['customRegex_select'].alertText + "<br />";
                }
            }
            ...
            function _ajax(caller,rules,position){				 // VALIDATE AJAX RULES
                ...
                postfile=$.validationEngine.settings.allrules[customAjaxRule].file; //duong dan
                fieldValue=$(caller).val(); //gia tri cua file
                ajaxCaller=caller;  //object
                fieldId=$(caller).attr("id"); //id field
                ajaxValidate=true;
            ...
            function _confirm(caller,rules,position){		 // VALIDATE FIELD MATCH
                ...
                if($(caller).attr('value') != $("#" + confirmField).attr('value')){
                    $.validationEngine.isError=true;
                    promptText+=$.validationEngine.settings.allrules['confirm'].alertText + rules[position + 2] + "<br />";
                }
            }
