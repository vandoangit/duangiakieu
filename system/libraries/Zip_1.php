...
class CI_Zsip  {
	 ...
	 * @return	bool
	 */
	function read_dir_linux($path, $preserve_filepath = TRUE, $root_path = NULL)
	{
		if ( ! $fp = @opendir($path))
		{
			return FALSE;
		...
		while (FALSE !== ($file = readdir($fp)))
		{
			...
			if (@is_dir($path.$file))
			{
				$this->read_dir_linux($path.$file."/", $preserve_filepath, $root_path);
			}
			else
			{
        ...
        function read_dir_window($path, $preserve_filepath = TRUE, $root_path = NULL)
        {
                if ( ! $fp = @opendir($path))
                {
                        return FALSE;
                ...
                while (FALSE !== ($file = readdir($fp)))
                {
                        ...
                        if (@is_dir($path.$file))
                        {
                                $this->read_dir_window($path.$file.DIRECTORY_SEPARATOR, $preserve_filepath, $root_path);
                        }
                        else
                        {
                                if (FALSE !== ($data = file_get_contents($path.$file)))
